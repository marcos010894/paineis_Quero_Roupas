         ...
         doc.data().products.forEach((e)=>{
            ...
            <div class="pedidomodal" style="border:1px solid green">
                <div class="conteudo" style="width:100!important">
                  ...
                  <button class="btn btn-success" onclick="aceitarMo('${doc.id}', '${valorfinal.toFixed(2)}', '${doc.id}')">Aceitar!</button>  <br>   <br>    
...
function aceitarMo(idPedido, price, uidPedido){
    ...
    db.collection("shops").doc(id).collection("orders").doc(idPedido).update({
      ...
      db.collection("shops").doc(id).collection("graphics").doc(uidPedido).set({
          price: total,
          data: dataAtual1,
          ano: ano1,
          mes:mes1,
  ...
  db.collection("shops").doc(id).onSnapshot((doc) => {
    console.log(doc.data().diaVencimento)
    if(doc.data().diaVencimento == dia1){
      $('#finaceiroModal').modal()
    }
  })
  function mensalidade(){  
    db.collection('shops').doc(id).collection('graphics').onSnapshot((querySnapshot) =>{
      querySnapshot.forEach ((doc) => {
        if(doc.data().mes == mes1 && doc.data().ano == ano1){
          valormes += somames + doc.data().price;
          if(valormes <= 3000 ){
          ...
          body = {
            ...
            "back_urls": {
              "success": "https://painel.queroapps.com/notification.php",
              "failure": "https://painel.queroapps.com/notification.php",
              "pending": "https://painel.queroapps.com/notification.php"
            },
            "auto_return": "all"
        };
        }
  ...
  $('#Pagar').click(function (){
    if(diaVencimentoLoja > dia1){
      alert('Ops, parece que a sua fatura ainda não venceu. Por favor tente pagar no dia ' + diaVencimentoLoja)
    }else{
    $.ajax({
          type: "POST",
          dataType: "json",
          contentType: 'application/json',
          url: "https://queroappspayment.herokuapp.com/payment",
          data: JSON.stringify(body),
					success:function(r){ 
            if(r.status == 200){           
              console.log(r.response)
              $('#iframe').append(`Aguarde carregando dados ...`)
              setTimeout(function(){ 
                $('#iframe').html(`<iframe src="${r.response.init_point}" width="100%" height="480" allowfullscreen></iframe>`)
               }, 5000);
...
db.collection("shops").doc(id)
        .onSnapshot((doc) => {
            ...
            diaVencimentoLoja = doc.data().diaVencimento
